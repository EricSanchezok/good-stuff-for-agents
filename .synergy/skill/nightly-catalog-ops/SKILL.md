...
7. **Commit and push only when authorized.** You inspect status/diff, exclude unrelated user changes, commit with the required co-author footer, and push normally. Never force push.
8. **Handoff or stop.** If the run is blocked, state the owner and exact next action. If complete, state the commit/push state and next-run priorities.
